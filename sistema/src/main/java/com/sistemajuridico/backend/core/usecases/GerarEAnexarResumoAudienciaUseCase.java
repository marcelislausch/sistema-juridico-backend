...
public class GerarEAnexarResumoAudienciaUseCase {
    ...
    private static final Logger logger = LoggerFactory.getLogger(GerarEAnexarResumoAudienciaUseCase.class);
    ...
    public Audiencia executar(UUID audienciaId, List<String> documentosIds) {
        if (documentosIds == null || documentosIds.isEmpty()) {
            throw new RegraNegocioException("A lista de IDs de documentos é obrigatória para a geração do resumo pela IA.");
        }
        ...
        for (int i = 0; i < documentosIds.size(); i++) {
            ...
            try {
                ...
                // Não é UUID, docId já é o ID direto do arquivo no Google Drive
            }
        ...
        if (textoConsolidado.length() == 0) {
            throw new RegraNegocioException("Nenhum conteúdo textual legível pôde ser extraído dos documentos informados.");
        }
        ...
        if (chunks.size() <= 1) {
            ...
            logger.info("Processo extenso detectado para audiência {} ({} caracteres). Fatiando em {} chunks para consolidação progressiva.",
                    audienciaId, textoConsolidadoStr.length(), chunks.size());
        ...
        ResumoAudienciaEstruturadoDTO resumoGerado = this.resumoAIService.resumirPecas(textoFinalParaResumo);
        try {
            String jsonResumo = this.objectMapper.writeValueAsString(resumoGerado);
            audiencia.setResumoPreparatorioIa(jsonResumo);
        } catch (JsonProcessingException ex) {
            throw new RegraNegocioException("Falha ao serializar o resumo estruturado para persistência: " + ex.getMessage());
        }
