# Documento de Especificação de Software (PRD) - v2.1
...
*   **IAM (Identity & Access Management):** Controle de usuários, perfis (Admin, Advogado, Secretária) e autenticação via JWT.
*   **Core Legal:** Gestão de clientes, processos judiciais e andamentos (linha do tempo).
*   **Financeiro & Agenda:** Controle de honorários, custas, despesas gerais do escritório e audiências.
...
| :--- | :--- | :--- | :--- |
| **Usuario** | `id`, `nome`, `email`, `senhaHash`, `perfil` (Enum: ADMIN, ADVOGADO, SECRETARIA), `oab`, `ativo` | 1:N Processos <br> 1:N Auditorias | O e-mail deve ser único. Apenas perfis ADVOGADO e ADMIN podem arquivar processos. |
...
| **Faturamento**| `id`, `tipo` (Enum: HONORARIOS, CUSTAS, DESPESAS_ESCRITORIO), `natureza` (Enum: A_RECEBER, A_PAGAR), `valor`, `dataVencimento`, `dataPagamento`, `status` (Enum: PENDENTE, PAGO, CANCELADO) | N:1 Processo (Opcional) | Ao marcar como PAGO, o sistema deve exigir a data efetiva do pagamento. |
...
*   **Tratamento de Erros:** Exceções globais interceptadas via `@RestControllerAdvice` para padronização de respostas HTTP 400/409.
...
## 4. Estrutura de Pastas (Clean Architecture)
    ...
    │   │   └── enums/              # PerfilAcesso, TipoFaturamentoEnum...
