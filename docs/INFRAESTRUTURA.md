...
## 7. Manual de Operação da VPS (Comandos Úteis)

O backend roda em um serviço gerenciado pelo `systemd` e operado pelo usuário restrito `appuser`[cite: 1]. Os segredos ficam isolados da aplicação.

**Acesso SSH:**
`ssh root@163.176.239.38` (ou seu IP de produção atual)

**Estrutura de Diretórios:**
* Raiz da aplicação: `/opt/sistema-juridico`
* Cofre de Segredos: `/opt/sistema-juridico/.env`
* Arquivo de Estrutura: `/opt/sistema-juridico/application.properties`
* Build compilado: `/opt/sistema-juridico/app.jar`

**Comandos Essenciais de Manutenção:**

* **Ver os logs em tempo real (Monitoramento):**
  `journalctl -u sistema-juridico.service -f`

* **Reiniciar o sistema (Após alterar variáveis no .env ou .properties):**
  `sudo systemctl restart sistema-juridico`

* **Verificar o status do serviço:**
  `systemctl status sistema-juridico`

* **Editar os Segredos de Produção (Senhas e Tokens):**
  `sudo nano /opt/sistema-juridico/.env`

* **Editar as Configurações Estruturais:**
  `sudo nano /opt/sistema-juridico/application.properties`